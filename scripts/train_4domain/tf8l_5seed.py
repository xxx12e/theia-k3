#!/usr/bin/env python
"""
Transformer 8L8H 5-seed wrapper, param-matched to THEIA (paper §4.2, Table 1 baseline).

Usage:
    python tf8l_5seed.py --seed 42
"""
...
p.add_argument('--output-root', type=str,
               default=r'multi_seed_results\tf8l')
args = p.parse_args()
...
print(f"TF8L | seed={SEED} | device={DEVICE}")
log(f"TF8L | seed={SEED}")
...
pbar=tqdm(range(1,MAX_EPOCHS+1),desc=f'TF8L s{SEED}',ncols=110)
for epoch in pbar:
    model.train()
    ...
    if epoch%20==0 and best_acc>0.98:
        ...
        # TF unlikely to hit 12/12; early stop on stability instead
        if best_acc>0.999 and kp==prev_kleene_passed:
            stable_count+=1
            if stable_count>=3 and epoch>=80:
...
summary={
    'seed':SEED,'model':'tf8l','params':params,
...
print(f"\n{'='*60}")
print(f"TF8L seed={SEED} | acc={best_acc:.4f} | {kleene_passed}/12 | {(time.time()-t0)/60:.1f}m")
