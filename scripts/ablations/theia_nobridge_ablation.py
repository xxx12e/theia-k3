#!/usr/bin/env python
"""
THEIA no-bridge ablation: remove the cross-domain bridge layers (bridge_ao,
bridge_as; 33K params, 1.2% of total). If this still reaches 12/12, the bridges
are a convergence aid, not a necessity (paper Sec. 4 ablations).
...
Usage:
    python theia_nobridge_ablation.py --seed 123

Outputs to: multi_seed_results\\ablations\\nobridge\\seed_{N}\\
"""
...
p.add_argument('--output-root', type=str,
               default=r'multi_seed_results\ablations\nobridge')
args = p.parse_args()
...
print(f"THEIA-NoBridge | seed={SEED} | device={DEVICE}")
log(f"THEIA-NoBridge | seed={SEED}")

# --- Model (isis_v9 verbatim, except bridges removed) ---
...
class IsisV9(nn.Module):
    def __init__(self):
        ...
        # ABLATION: bridge layers removed (set to identity)
        self.bridge_ao = nn.Identity()
        self.bridge_as = nn.Identity()
    ...
    def forward(self, a,b,d,set_bits,s_unk,a_unk,b_unk,d_unk,arith,rel,op):
        ...
        # ABLATION: c_vec passed directly without bridge transformation
        ord_vec = self.order_eng(c_vec, d, d_unk, rel)
        set_vec = self.set_eng(c_vec, set_bits, s_unk)
...
pbar = tqdm(range(1, MAX_EPOCHS+1), desc=f'THEIA-NoBridge s{SEED}', ncols=110)
for epoch in pbar:
    model.train()
...
summary = {
    'seed': SEED, 'model': 'theia_nobridge_ablation', 'params': params,
...
print(f"\n{'='*60}")
print(f"THEIA-NoBridge seed={SEED} | acc={best_acc:.4f} | {kleene_passed}/12 | "
