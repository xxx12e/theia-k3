#!/usr/bin/env python
"""
THEIA subspace ablation: replace the parallel C/D/I Logic Engine subspaces with a
single capacity-matched MLP. If this still reaches 12/12, the modular pipeline
(not the subspace structure) is what matters (paper Sec. 4 ablations).
...
Usage:
    python theia_subspace_ablation.py --seed 123

Outputs to: multi_seed_results\\ablations\\subspace\\seed_{N}\\
"""
...
p.add_argument('--output-root', type=str,
               default=r'multi_seed_results\ablations\subspace')
args = p.parse_args()
...
print(f"THEIA-Subspace | seed={SEED} | device={DEVICE}")
log(f"THEIA-Subspace | seed={SEED}")

# --- Model (isis_v9 verbatim, except LogicEngine below) ---
...
class LogicEngine(nn.Module):
    """ABLATION: Replace parallel C/D/I subspaces with a single MLP.
    Designed to roughly match capacity (single MLP path 384->768->384->128).
    Total model params: ~2.45M (vs original 2.75M)."""
    def __init__(self):
        super().__init__()
        self.oe = nn.Embedding(N_OPS, D_MODEL)
        self.net = nn.Sequential(
            nn.Linear(D_MODEL*3, D_MODEL*6), nn.GELU(), nn.LayerNorm(D_MODEL*6),
            nn.Linear(D_MODEL*6, D_MODEL*3), nn.GELU(), nn.LayerNorm(D_MODEL*3),
...
pbar = tqdm(range(1, MAX_EPOCHS+1), desc=f'THEIA-Subspace s{SEED}', ncols=110)
for epoch in pbar:
    model.train()
...
summary = {
    'seed': SEED, 'model': 'theia_subspace_ablation', 'params': params,
...
print(f"\n{'='*60}")
print(f"THEIA-Subspace seed={SEED} | acc={best_acc:.4f} | {kleene_passed}/12 | "
