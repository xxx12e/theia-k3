"""
Deep MLP ablation: 12-layer flat MLP, parameter-matched to THEIA (~2.75M).
HIDDEN=472, N_LAYERS=12 -> total ~2,746,502 params (diff -4730 from THEIA).
Tests whether DEPTH rescues flat-MLP chain generalization.
"""
...
# --- Parameter-matched MLP step computer (HIDDEN=472, 12 layers -> ~2.75M) ---
DIM_NUM = 128; DIM_EMB = 128; DIM_SET = 128
HIDDEN = 472    # 12L param-matched: total ~2,746,502
N_LAYERS = 12
...
class MatchedMLPStepComputer(nn.Module):
    """Flat MLP with HIDDEN=472, N_LAYERS=12 to match THEIA's 2.75M param count."""
    def __init__(self):
        super().__init__()
...
def main():
    ...
    OUT_DIR = "multi_seed_results/matched_mlp_12L_backbone_ablation"
    os.makedirs(OUT_DIR, exist_ok=True)

    print(f"\n{'='*64}")
    print(f"  Deep MLP Ablation: 12-Layer (HIDDEN={HIDDEN})")
    print(f"  Seeds: {SEEDS}")
    ...
    # --- Report ---
    lines = []
    lines.append("# Deep MLP Ablation: 12-Layer, Parameter-Matched\n")
