#!/usr/bin/env python
"""
THEIA P_unk generalization ablation: train at P_unk=0.05, evaluate at P_unk=0.50
(§4.3). Tests whether a model trained on rare Unknown generalizes to majority
Unknown. Reports (1) the 12-rule Kleene diagnostic and (2) overall accuracy on a
P_unk=0.50 held-out set.

Kleene diagnostic is doubly fixed: Fix #1 (vo==2 sets du, not au, to avoid
c_unknown pollution); Fix #2 (vo==1 uses REL_GTE with d=max(0,c-1) to avoid the
c=0 edge case).

Usage:
    python theia_punk005_ablation.py --seed 123

Outputs to: multi_seed_results\\ablations\\punk005\\seed_{N}\\
"""
...
p.add_argument('--output-root', type=str,
               default=r'multi_seed_results\ablations\punk005')
args = p.parse_args()
...
D_MODEL=128; BATCH=4096; NUM_RANGE=20; SET_DIM=21; P_UNKNOWN=0.05  # ABLATION: train at low Unknown
...
print(f"THEIA-Punk005 | seed={SEED} | device={DEVICE}")
log(f"THEIA-Punk005 | seed={SEED}")
...
# --- Kleene diagnostic (doubly-fixed: Fix #1 du-injection, Fix #2 REL_GTE) ---
KLEENE_TESTS = [
...
pbar = tqdm(range(1, MAX_EPOCHS+1), desc=f'THEIA-Punk005 s{SEED}', ncols=110)
for epoch in pbar:
    model.train()
...
# --- Ablation-specific: evaluate on P_unk=0.50 held-out set ---
print("evaluating on Punk=0.50 held-out set...")
def build_test_set_punk050(seed):
    torch.manual_seed(seed + 9999)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed + 9999)
    N_t = 200_000
    P50 = 0.50
    a_v = torch.randint(1, NUM_RANGE+1, (N_t,), device=DEVICE)
    b_v = torch.randint(1, NUM_RANGE+1, (N_t,), device=DEVICE)
    d_v = torch.randint(0, NUM_RANGE+1, (N_t,), device=DEVICE)
    arith_v = torch.randint(0, N_ARITH, (N_t,), device=DEVICE)
    rel_v   = torch.randint(0, N_RELS,  (N_t,), device=DEVICE)
    op_v    = torch.randint(0, N_OPS,   (N_t,), device=DEVICE)
    a_u = torch.rand(N_t, device=DEVICE) < P50
    b_u = torch.rand(N_t, device=DEVICE) < P50
    d_u = torch.rand(N_t, device=DEVICE) < P50
    s_u = torch.rand(N_t, device=DEVICE) < P50
    c_v = torch.zeros(N_t, dtype=torch.long, device=DEVICE)
    c_v[arith_v==ARITH_ADD] = torch.clamp(a_v+b_v,0,NUM_RANGE)[arith_v==ARITH_ADD]
    c_v[arith_v==ARITH_SUB] = torch.abs(a_v-b_v)[arith_v==ARITH_SUB]
    c_v[arith_v==ARITH_MUL] = torch.clamp(a_v*b_v,0,NUM_RANGE)[arith_v==ARITH_MUL]
    c_v[arith_v==ARITH_MOD] = (a_v % torch.clamp(b_v,1,NUM_RANGE))[arith_v==ARITH_MOD]
    c_v = torch.clamp(c_v,0,NUM_RANGE)
    c_unk = a_u | b_u
    ord_unk = c_unk | d_u
    ord_v = torch.zeros(N_t, dtype=torch.long, device=DEVICE)
    rt = (((rel_v==0)&(c_v>d_v))|((rel_v==1)&(c_v<d_v))|
          ((rel_v==2)&(c_v==d_v))|((rel_v==3)&(c_v>=d_v))|
          ((rel_v==4)&(c_v<=d_v))|((rel_v==5)&(c_v!=d_v)))
    ord_v[rt] = VAL_TRUE
    val_o = torch.where(ord_unk, torch.tensor(VAL_UNKNOWN,device=DEVICE), ord_v)
    sb_v = torch.randint(0, 2, (N_t,SET_DIM), dtype=torch.float32, device=DEVICE)
    sou = s_u | c_unk
    ci = c_v.clamp(0, SET_DIM-1)
    ins = sb_v[torch.arange(N_t,device=DEVICE), ci].bool()
    sv = torch.where(ins, torch.tensor(VAL_TRUE,device=DEVICE),
                          torch.tensor(VAL_FALSE,device=DEVICE))
    val_s = torch.where(sou, torch.tensor(VAL_UNKNOWN,device=DEVICE), sv)
    tgt = apply_logic(op_v, val_o, val_s)
    return (a_v.float()/NUM_RANGE, b_v.float()/NUM_RANGE, d_v.float()/NUM_RANGE,
            sb_v, s_u, a_u, b_u, d_u, arith_v, rel_v, op_v, tgt)

test_data = build_test_set_punk050(SEED)
test_tgt = test_data[-1]
test_inputs = test_data[:-1]
N_t = test_tgt.shape[0]
model.eval()
with torch.no_grad():
    preds_p50 = []
    for j in range(0, N_t, BATCH*4):
        bi_inputs = tuple(t[j:j+BATCH*4] for t in test_inputs)
        with torch.amp.autocast('cuda'):
            preds_p50.append(model.classify(model(*bi_inputs)))
    preds_p50 = torch.cat(preds_p50)
    p50_overall_acc = (preds_p50 == test_tgt).float().mean().item()
    p50_per_class = {}
    for vid, vn in [(0,'False'),(1,'True'),(2,'Unknown')]:
        m = test_tgt == vid
        if m.sum() > 0:
            p50_per_class[vn] = (preds_p50[m]==test_tgt[m]).float().mean().item()
log(f"P50 eval: overall={p50_overall_acc:.4f} per_class={p50_per_class}")
print(f"  P_unk=0.50 overall: {p50_overall_acc:.4f}")
print(f"  P_unk=0.50 per-class: {p50_per_class}")
...
summary = {
    'seed': SEED, 'model': 'theia_punk005_ablation', 'params': params,
    'P_unk_train': 0.05, 'P_unk_test': 0.50,
    'overall_acc_p015_train': best_acc,
    'overall_acc_p050_test': p50_overall_acc,
    'p050_per_class': p50_per_class,
    'converge_epoch': converge_epoch, 'max_epochs': MAX_EPOCHS,
...
print(f"\n{'='*60}")
print(f"THEIA-Punk005 seed={SEED} | acc={best_acc:.4f} | {kleene_passed}/12 | "
