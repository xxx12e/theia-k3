"""
Deep MLP ablation: 6-layer flat MLP, parameter-matched to THEIA (~2.75M).
HIDDEN=711, N_LAYERS=6 -> total ~2,751,652 params (diff +420 from THEIA).
...
# --- Parameter-matched MLP step computer (HIDDEN=711, 6 layers -> ~2.75M) ---
DIM_NUM = 128; DIM_EMB = 128; DIM_SET = 128
HIDDEN = 711    # 6L param-matched: total ~2,751,652
N_LAYERS = 6
...
class MatchedMLPStepComputer(nn.Module):
    """Flat MLP with HIDDEN=711, N_LAYERS=6 to match THEIA's 2.75M param count."""
    def __init__(self):
        super().__init__()
...
def main():
    ...
    OUT_DIR = "multi_seed_results/matched_mlp_6L_backbone_ablation"
    os.makedirs(OUT_DIR, exist_ok=True)

    print(f"\n{'='*64}")
    print(f"  Deep MLP Ablation: 6-Layer (HIDDEN={HIDDEN})")
    print(f"  Seeds: {SEEDS}")
    ...
    # --- Report ---
    lines = []
    lines.append("# Deep MLP Ablation: 6-Layer, Parameter-Matched\n")
